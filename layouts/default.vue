  ...
  <div class="min-h-screen flex flex-col bg-slate-50 pb-20">
    ...
    <header class="sticky top-0 z-40 bg-white border-b border-slate-100 shadow-sm">
      <div class="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
        <!-- Logo -->
        <NuxtLink to="/" class="flex items-center gap-2 font-black text-sky-500 text-xl tracking-tight">
    ...
    <nav class="fixed bottom-0 inset-x-0 z-40 bg-white border-t border-slate-100 shadow-[0_-1px_8px_rgba(0,0,0,0.06)]">
      <div class="max-w-lg mx-auto flex">
        ...
        <NavItem to="/learn" icon="📖" label="Aprender" />
        <NavItem to="/achievements" icon="🏆" label="Logros" />
        <NavItem to="/profile" icon="👤" label="Perfil" />
      </div>
    </nav>

    <!-- Level-up modal -->
    <LevelUpModal v-if="userStore.justLeveledUp" @close="userStore.clearLevelUp()" />
  </div>
</template>
