<template>
  <div class="space-y-5">
    ...
    <div class="card flex flex-col items-center py-8 gap-3">
      ...
      <h2 class="text-xl font-black text-slate-800">{{ userStore.name }}</h2>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="card text-center">
        ...
        <div class="text-2xl font-black text-orange-500">{{ userStore.streak }}</div>
        <div class="text-xs text-slate-500 font-semibold mt-0.5">Racha actual</div>
      </div>
      <div class="card text-center">
        <div class="text-2xl mb-1">📅</div>
        <div class="text-2xl font-black text-amber-500">{{ userStore.maxStreak }}</div>
        <div class="text-xs text-slate-500 font-semibold mt-0.5">Racha máxima</div>
      </div>
      <div class="card text-center">
        <div class="text-2xl mb-1">⚡</div>
        <div class="text-2xl font-black text-sky-500">{{ userStore.xp }}</div>
        <div class="text-xs text-slate-500 font-semibold mt-0.5">XP total</div>
      </div>
      <div class="card text-center">
        <div class="text-2xl mb-1">📚</div>
        <div class="text-2xl font-black text-emerald-500">{{ progressStore.totalLessonsCompleted }}</div>
        <div class="text-xs text-slate-500 font-semibold mt-0.5">Lecciones</div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="flex items-center justify-between mb-3">
        <p class="font-black text-slate-700">Logros</p>
        <NuxtLink to="/achievements" class="text-xs text-sky-500 font-bold">Ver todos →</NuxtLink>
      </div>
      <div class="flex items-center gap-2">
